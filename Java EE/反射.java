 ...
 *
 */
public class BaseDao<T>{  // BaseDao<T,K>
...
'泛型反射
	...
	public BaseDao(){
		...
		//                                           其实就是“参数化类型”， ParameterizedType （ p unan mete zedi Type ）
								  （ get	Jnewe supe class ）
		Type type = this.getClass().getGenericSuperclass();   
		// 强制转换为“参数化类型”  【BaseDao<Account>】
		ParameterizedType pt = (ParameterizedType) type;
		
		// 获取参数化类型中，实际类型的定义  【new Type[]{Account.class}】
		Type types[] =  pt.getActualTypeArguments();// 强转  获得 Account 对象
